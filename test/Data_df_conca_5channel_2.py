...
class_weight = {0: 1.34, 1: 1.0, 2: 1.5}
...
with strategy.scope():
    ...
    history = model.fit(train_dataset.skip(round(0.1*len(tr_labels))).map(tf_random_rotate_image_xyz).shuffle(buffer_size=120).batch(_batch_size),
                             class_weight = None,
                             epochs=300,
                             verbose=1,
                             callbacks=callbacks,
...
from resnet3d import Resnet3DBuilder 
model = Resnet3DBuilder.build_resnet_50((128, 128, 128, 5), 1,reg_factor=1e-4)
