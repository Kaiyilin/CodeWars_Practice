...
from Grad_CAM_3D_functions import Grad_CAM_function
...
BA_images_tr = np.concatenate([BA_alff_tr, BA_gfa_tr, BA_iso_tr],axis=-1)
BB_images_tr = np.concatenate([BB_alff_tr, BB_gfa_tr, BB_iso_tr],axis=-1)
HC_images_tr = np.concatenate([HC_alff_tr, HC_gfa_tr, HC_iso_tr],axis=-1)
...
BA_images_val = np.concatenate([BA_alff_val, BA_gfa_val, BA_iso_val],axis=-1)
BB_images_val = np.concatenate([BB_alff_val, BB_gfa_val, BB_iso_val],axis=-1)
HC_images_val = np.concatenate([HC_alff_val, HC_gfa_val, HC_iso_val],axis=-1)
...
with strategy.scope():
    model = Resnet3DBuilder.build_resnet_50((128, 128, 128, 3), 3,reg_factor=1e-4)
    ...
    history = model.fit( ds_tr.map(tf_random_rotate_image).shuffle(buffer_size=_buffer_size).batch(_batch_size),
                                   class_weight = None,
                                   epochs = 150,
                                   verbose = 1,
                                   callbacks = callbacks,
